...
#' @param initializer_name A string indicating the name of the initializer method.
...
estimator_and_initializer_string_to_node_class_type <- function(estimator_name, initializer_name){
  ...
  if(tolower(initializer_name) == "normalpls"){
    return(PLSNode)
  }
    ...
    stop("Supplied initializer/estimator name is not supported.")
  }
