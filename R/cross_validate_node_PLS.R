...
#' @param max_n_LVs An integer indicating the max number of LVs that are to be evaluated.
#' @param k_folds An integer indicating the number of folds that are to be evaluated for the k-fold cross validation. It can not be higher than the number of samples.
#' @param error_function A function handle for an error function which takes two equal size matrices and returns a single double.
#' @return A Matrix of errors for each fold and each number of LVs. The rows indicate the fold, the columns indicate the max number of LVs.
#'
